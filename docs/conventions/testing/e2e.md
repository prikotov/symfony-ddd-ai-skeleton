...
Контейнеры E2E окружения имеют префикс compose-проекта:
- вне `Sandbox` по умолчанию это `task-e2e`;
- в `Sandbox` по умолчанию это `task-e2e-<sandbox-slug>`;
- точное значение покажет `make e2e-project-name`.

Вы можете смотреть логи запущенных контейнеров с помощью `podman logs` или `docker logs`.

```bash
E2E_PROJECT_NAME=$(make e2e-project-name)

# Логи backend приложения (PHP) - ошибки 500, исключения
podman logs ${E2E_PROJECT_NAME}-php-fpm-1

# Логи веб-сервера (Nginx) - access logs, коды ответов
podman logs ${E2E_PROJECT_NAME}-nginx-1

# Логи worker обработки очередей (важно для Source Pipeline)
podman logs ${E2E_PROJECT_NAME}-worker-cli-1

# Логи выполнения тестов (PHPUnit output)
podman logs ${E2E_PROJECT_NAME}-test-runner-1

# Логи браузера Selenium (Chrome) - ошибки драйвера
podman logs ${E2E_PROJECT_NAME}-selenium-1

# Логи PostgreSQL и RabbitMQ
podman logs ${E2E_PROJECT_NAME}-database-1
...
```bash
E2E_PROJECT_NAME=$(make e2e-project-name)
podman compose -p "${E2E_PROJECT_NAME}" exec test-runner \
  curl -fsS http://mailer:8025/api/v1/messages | jq
```
