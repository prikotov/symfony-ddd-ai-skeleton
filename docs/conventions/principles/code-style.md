...
- Проект следует стандарту PSR-12 как базовому.
- Используется `declare(strict_types=1)` во всех PHP-файлах.
- PHP 8.4+: типизированные свойства, readonly, enum, named arguments.
- Читаемость важнее краткости: осмысленные имена, минимальная вложенность.

## Расположение

- Конвенции применяются ко всем файлам в `src/`, `tests/`, `apps/`.
- Проверка стиля: PHP_CodeSniffer через `make phpcs`.
- Конфигурация: [`phpcs.xml.dist`](../examples/phpcs.xml.dist).
