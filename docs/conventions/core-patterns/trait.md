...
- Трейт автономен: не требует методов/свойств хоста, не опирается на окружение, не тянет инфраструктуру. Если трейт становится "общим" — переносите его в `ProjectName\Common\Component\*`.
...
```
{ProjectName}\Common\Component\{ComponentName}\Trait\{Name}Trait
```

- Трейты для консольного приложения:

```
{ProjectName}\Console\Module\{ModuleName}\Trait\{Name}Trait
```

- Трейты модулей:

```
{ProjectName}\Common\Module\{ModuleName}\{Layer}\Trait\{Name}Trait
```
...
namespace ProjectName\Common\Component\Repository\Trait;

use ProjectName\Common\Component\Repository\Enum\SortEnum;
...
namespace ProjectName\Common\Component\Doctrine\Trait;
