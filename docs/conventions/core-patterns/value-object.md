...
- Константы (`const`) допускаются — `public` для доменных ограничений (лимиты, точность), `private` для внутренних деталей реализации.
...
- Публичные статические методы — только фабрики `create*()` (например, `createFromEmail()`, `createDefault()`, `createEmpty()`).
...
- Запрещены побочные эффекты, I/O и traits; допускаются только чистые операции над собственными значениями.
...
- Инстанс-методы — чистые операции над собственными значениями. Метод либо возвращает значение свойства (геттер), либо вычисляет и возвращает новый VO (`add()`, `merge()`, `with*()`), либо преобразует в другой тип (`toString()`, `toArray()`). Побочные эффекты запрещены.
