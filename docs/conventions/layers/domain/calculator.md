...
name: Calculator
type: rule
description: Правила создания калькуляторов для математических и временных вычислений
---
...
```php
{ProjectName}\Common\Module\{ModuleName}\Domain\Calculator\{GroupName}\{Name}Calculator
```
...
```php
<?php
namespace ProjectName\Common\Module\Rag\Domain\Calculator\Vector;
...
namespace ProjectName\Common\Module\Billing\Domain\Calculator;

use ProjectName\Common\Module\Billing\Domain\Entity\PromoCodeModel;
use ProjectName\Common\Module\Billing\Domain\Enum\PromoCodeDiscountTypeEnum;
